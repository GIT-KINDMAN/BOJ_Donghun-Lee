...
public class Main2 {
	...
	static int count=1; // 시작점인 (1,1)은 카운트된 상태로 시작하기 때문에 1로 초기화
	static int n,m;
	...
	public static void bfs(int a, int b) { //BFS메소드
		...
		b-=1;
		check[a][b]= count; // 초기 count변수 값을 방문배열에서 시작점에 저장한다.
		
		queue.offer(new Point(a,b)); // 시작점의 객체를 큐에 삽입
		while(!queue.isEmpty()) { // 이제 큐가 공백이 될 때 까지 반복한다.
			...
			queue.poll(); // 큐에서 peek후 저장한 것이기 때문에 별도이 poll()수행으로 해당 객체 소멸
					...
					if(y<node[x].length-1&&node[x][y]==1&&node[x][y+1]==1 && check[x][y+1]==0) {
						...
						queue.offer(new Point(x,y+1)); // 인접한 점을 큐에 넣어준다.
					//아래는 모두 이와 동일
					}
